...
export function RegisterForm({ onSubmit, loading = false }) {
  ...
  return <form onSubmit={(e) => { e.preventDefault(); onSubmit(form); }} className="space-y-5">
    ...
    <div><label className="field-label" htmlFor="reg-password">Пароль</label><input id="reg-password" required minLength="8" type="password" autoComplete="new-password" className="field-control" placeholder="Минимум 8 символов" value={form.password} onChange={(e) => setForm({ ...form, password: e.target.value })} /></div>
    ...
    <Button type="submit" loading={loading} className="w-full">Создать аккаунт</Button>
  </form>;
}
