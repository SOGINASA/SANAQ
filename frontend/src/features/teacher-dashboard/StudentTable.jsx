  ...
  return <>
    <div className="grid gap-3 sm:hidden">
      {rows.map((student) => {
        ...
        return <button key={student.id} type="button" onClick={() => openProfile(student)} className="w-full cursor-pointer rounded-2xl border border-stone-200 bg-paper p-4 text-left transition active:bg-lavender-50" aria-label={t('studentTable.openProfile', { name: student.name })}>
          ...
          <span className="mt-4 grid grid-cols-2 gap-2 text-sm"><span className="rounded-xl bg-stone-100 p-3"><span className="block text-xs text-stone-500">{t('studentTable.progress')}</span><strong className="mt-1 block tabular-nums">{student.progress}%</strong></span><span className="rounded-xl bg-stone-100 p-3"><span className="block text-xs text-stone-500">{t('studentTable.activeDays')}</span><strong className="mt-1 block tabular-nums">{student.streak}</strong></span></span>
          <span className={`mt-3 inline-flex rounded-full px-3 py-1.5 text-xs font-bold ${risk[1]}`}>{risk[0]}</span>
        </button>;
      })}
    </div>
    <div className="hidden overflow-x-auto sm:block"><table className="w-full min-w-[680px] border-collapse text-left"><thead><tr className="border-b border-stone-200 text-xs uppercase tracking-wider text-stone-400"><th className="px-3 py-4">{t('studentTable.student')}</th><th className="px-3 py-4">{t('studentTable.progress')}</th><th className="px-3 py-4">{t('studentTable.activeDays')}</th><th className="px-3 py-4">{t('studentTable.focus')}</th><th className="px-3 py-4">{t('studentTable.status')}</th><th aria-label={t('studentTable.action')} /></tr></thead><tbody>{rows.map((student) => { const risk = riskLabel[student.risk] || riskLabel.attention; return <tr key={student.id} className="border-b border-stone-100 last:border-0"><td className="px-3 py-4 font-bold">{student.name}</td><td className="px-3 py-4 tabular-nums">{student.progress}%</td><td className="px-3 py-4 tabular-nums">{student.streak}</td><td className="px-3 py-4 text-sm text-stone-600">{student.focus}</td><td className="px-3 py-4"><span className={`rounded-full px-3 py-1.5 text-xs font-bold ${risk[1]}`}>{risk[0]}</span></td><td className="px-3 py-4"><button onClick={() => openProfile(student)} className="grid h-11 w-11 cursor-pointer place-items-center rounded-xl hover:bg-stone-100" aria-label={t('studentTable.openProfile', { name: student.name })}><ChevronRight className="h-5 w-5" /></button></td></tr>; })}</tbody></table></div>
  </>;
}
