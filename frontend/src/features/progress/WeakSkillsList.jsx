...
export function WeakSkillsList() {
  ...
  return <div className="space-y-3">{skills.map((skill, index) => <div key={skill.title} className="flex items-center gap-3 rounded-2xl border border-stone-200 p-4"><span className={`grid h-10 w-10 shrink-0 place-items-center rounded-xl ${index === 0 ? 'bg-[#FFE8E2] text-[#A74735]' : 'bg-stone-100 text-stone-600'}`}><AlertCircle className="h-5 w-5" /></span><div className="min-w-0 flex-1"><p className="truncate font-bold">{skill.title}</p><p className="text-sm text-stone-500">Освоено на {skill.value}%</p></div><button className="grid h-11 w-11 shrink-0 place-items-center rounded-xl bg-stone-100" aria-label={`${skill.action}: ${skill.title}`}><ArrowUpRight className="h-5 w-5" /></button></div>)}</div>;
}
