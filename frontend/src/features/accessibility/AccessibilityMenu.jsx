...
export function AccessibilityMenu() {
  ...
  return <><button type="button" onClick={() => setOpen(true)} className="fixed bottom-[calc(5.5rem+env(safe-area-inset-bottom))] right-3 z-40 grid h-12 w-12 cursor-pointer place-items-center rounded-2xl border border-lavender-200 bg-paper text-lavender-700 shadow-soft transition hover:bg-lavender-50 sm:right-4 lg:bottom-6" aria-label={t('accessibility.open')}><Accessibility className="h-6 w-6" /></button><Dialog open={open} onClose={() => setOpen(false)} title={t('accessibility.title')} description={t('accessibility.description')} footer={<Button variant="ghost" onClick={() => setOpen(false)}>{t('accessibility.done')}</Button>}>
