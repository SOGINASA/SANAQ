...
const triggerPosition = {
  ...
  assistant: 'right-3 top-[calc(4.5rem+env(safe-area-inset-top))] sm:right-4 lg:bottom-6 lg:right-6 lg:top-auto',
  public: 'bottom-4 right-3 sm:bottom-6 sm:right-6',
};

export function AccessibilityMenu({ context = 'public' }) {
  const [open, setOpen] = useState(false);
  const { t } = useI18n();
  const store = useAccessibilityStore();
  const pageText = () => document.querySelector('main')?.innerText || document.body.innerText;
  return <><button type="button" onClick={() => setOpen(true)} className={`fixed z-40 grid h-12 w-12 cursor-pointer place-items-center rounded-2xl border border-lavender-200 bg-paper text-lavender-700 shadow-soft transition hover:bg-lavender-50 ${triggerPosition[context] || triggerPosition.public}`} aria-label={t('accessibility.open')}><Accessibility className="h-6 w-6" /></button><Dialog open={open} onClose={() => setOpen(false)} title={t('accessibility.title')} description={t('accessibility.description')} footer={<Button variant="ghost" onClick={() => setOpen(false)}>{t('accessibility.done')}</Button>}>
