...
import { BookOpen, LayoutDashboard, MessageCircleQuestion, LibraryBig, School, Users, Activity } from 'lucide-react';
import { cn } from '../../shared/lib/cn';
import { useI18n } from '../../shared/i18n/i18n';

export function MobileNavigation({ role = 'student' }) {
  const { t } = useI18n();
  const items = role === 'admin' ? [['nav.overview', '/admin/dashboard', LayoutDashboard], ['nav.users', '/admin/users', Users], ['nav.content', '/admin/content', LibraryBig], ['nav.system', '/admin/system', Activity]] : role === 'teacher' ? [['nav.overview', '/teacher/dashboard', LayoutDashboard], ['nav.content', '/teacher/content', LibraryBig]] : [['nav.overview', '/student/dashboard', LayoutDashboard], ['nav.classroom', '/student/class', School], ['nav.path', '/student/path', BookOpen], ['nav.assistant', '/student/assistant', MessageCircleQuestion]];
