...
export function MobileNavigation({ role = 'student' }) {
  const { t } = useI18n();
  const items = role === 'teacher' ? [['nav.overview', '/teacher/dashboard', LayoutDashboard], ['nav.content', '/teacher/content', LibraryBig]] : [['nav.overview', '/student/dashboard', LayoutDashboard], ['nav.path', '/student/path', BookOpen], ['nav.assistant', '/student/assistant', MessageCircleQuestion]];
