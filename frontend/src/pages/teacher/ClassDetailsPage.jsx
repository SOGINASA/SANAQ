...
import { Button, Card, Dialog, ProgressBar, Skeleton, StatusToast, Tabs } from '../../shared/ui';
...
export function ClassDetailsPage() {
  ...
  const publishAnnouncement = async (event) => {
    ...
    try {
      ...
      setToast('Объявление опубликовано');
    } catch (requestError) { setError(requestError.message); }
  ...
  const createAssignment = async () => {
    ...
    try {
      ...
      setToast('Задание отправлено классу');
    } catch (requestError) { setError(requestError.message); }
    finally { setActionLoading(false); }
  };
  ...
  const tabs = [
    ...
    { value: 'students', label: `Ученики · ${students.length}` },
    ...
    <div className="mx-auto max-w-7xl animate-rise">
      ...
      <Card className="overflow-hidden border-0 bg-ink p-6 text-white sm:p-8">
        <div className="flex flex-col gap-6 lg:flex-row lg:items-end lg:justify-between">
          <div className="min-w-0"><p className="text-xs font-bold uppercase tracking-[0.16em] text-lime">Рабочее пространство класса</p><h1 className="mt-3 break-words text-3xl font-extrabold sm:text-4xl">{classroom?.name} · {classroom?.grade} класс</h1><p className="mt-3 text-sm text-stone-300">{students.length} учеников · Математика</p></div>
          <div className="flex flex-col gap-3 sm:flex-row">
            <button type="button" onClick={copyCode} className="flex min-h-12 items-center justify-between gap-5 rounded-2xl border border-stone-600 bg-stone-800 px-4 text-left transition hover:border-lavender-400"><span><span className="block text-[11px] font-bold uppercase tracking-wider text-stone-400">Код класса</span><strong className="mt-0.5 block tracking-[0.16em]">{classroom?.join_code}</strong></span><Copy className="h-5 w-5 text-lime" /></button>
            <Button variant="outline" onClick={exportClass}><Download className="h-4 w-4" /> {t('classDetails.export')}</Button>
          </div>
        </div>
      </Card>

      <Tabs className="mt-6 w-full sm:w-fit" items={tabs} value={activeTab} onChange={setActiveTab} label="Разделы класса" />
      ...
      {activeTab === 'feed' && <div className="mt-6 grid items-start gap-6 xl:grid-cols-[minmax(0,1fr)_320px]">
        <div className="min-w-0">
          <Card className="p-5 sm:p-6"><form onSubmit={publishAnnouncement}>
            <div className="flex items-center gap-3"><span className="grid h-11 w-11 place-items-center rounded-2xl bg-lavender-100 text-lavender-700"><Megaphone className="h-5 w-5" /></span><div><h2 className="font-extrabold">Объявление классу</h2><p className="text-sm text-stone-500">Все ученики получат уведомление</p></div></div>
            <div className="mt-5 grid gap-3"><label className="field-label">Заголовок<input className="field-control mt-2" maxLength="160" value={announcement.title} onChange={(event) => setAnnouncement({ ...announcement, title: event.target.value })} placeholder="Например, что повторить к пятнице" /></label><label className="field-label">Сообщение<textarea className="field-control mt-2 min-h-28 resize-y" maxLength="5000" value={announcement.body} onChange={(event) => setAnnouncement({ ...announcement, body: event.target.value })} placeholder="Напишите коротко и понятно…" /></label></div>
            <div className="mt-4 flex flex-col gap-3 sm:flex-row sm:items-center sm:justify-between"><label className="flex min-h-11 cursor-pointer items-center gap-3 rounded-xl px-1 text-sm font-semibold text-stone-600"><input type="checkbox" className="h-5 w-5 accent-lavender-600" checked={announcement.is_pinned} onChange={(event) => setAnnouncement({ ...announcement, is_pinned: event.target.checked })} /><Pin className="h-4 w-4" /> Закрепить наверху</label><Button type="submit" loading={actionLoading} disabled={!announcement.title.trim() || !announcement.body.trim()} className="w-full sm:w-auto">Опубликовать</Button></div>
          </form></Card>
          <div className="mb-4 mt-8 flex items-end justify-between gap-4"><div><p className="eyebrow">Последние события</p><h2 className="mt-2 text-2xl font-extrabold">Лента класса</h2></div><span className="hidden text-sm text-stone-500 sm:block">{announcements.length + assignments.length} публикаций</span></div>
        ...
        <Card className="p-5 xl:sticky xl:top-24"><span className="grid h-12 w-12 place-items-center rounded-2xl bg-mint-100 text-mint-700"><ClipboardPlus className="h-6 w-6" /></span><h2 className="mt-5 text-xl font-extrabold">Дать задание</h2><p className="mt-2 text-sm leading-6 text-stone-500">Выберите готовый модуль, укажите срок — задание сразу появится у учеников.</p><Button className="mt-5 w-full" onClick={() => setAssignmentOpen(true)} disabled={!modules.length}>Новое задание</Button>{!modules.length && <p className="mt-3 text-xs leading-5 text-stone-500">Сначала добавьте хотя бы один учебный модуль в разделе «Материалы».</p>}</Card>
      </div>}

      {activeTab === 'students' && <Card className="mt-6 p-5 sm:p-8"><div className="flex flex-col gap-4 sm:flex-row sm:items-center sm:justify-between"><div><p className="eyebrow">Участники</p><h2 className="mt-2 text-2xl font-extrabold">{t('classDetails.students')}</h2></div><label className="relative block"><span className="sr-only">{t('classDetails.search')}</span><Search className="absolute left-4 top-3.5 h-5 w-5 text-stone-400" /><input value={search} onChange={(event) => setSearch(event.target.value)} className="field-control pl-11 sm:w-72" placeholder={t('classDetails.searchPlaceholder')} /></label></div><div className="mt-5"><StudentTable students={students} search={search} /></div></Card>}
      ...
      <Dialog open={assignmentOpen} onClose={() => setAssignmentOpen(false)} title="Новое задание" description="Задание появится в ленте и уведомлениях всех учеников класса." footer={<><Button variant="ghost" onClick={() => setAssignmentOpen(false)}>Отмена</Button><Button loading={actionLoading} disabled={!assignment.title.trim() || !assignment.module_id} onClick={createAssignment}>Назначить</Button></>}><div className="grid gap-4"><label className="field-label">Название<input className="field-control mt-2" value={assignment.title} onChange={(event) => setAssignment({ ...assignment, title: event.target.value })} placeholder="Например, Повторить квадратные уравнения" /></label><label className="field-label">Учебный модуль<select className="field-control mt-2" value={assignment.module_id} onChange={(event) => setAssignment({ ...assignment, module_id: event.target.value })}>{modules.map((item) => <option key={item.id} value={item.id}>{item.title}</option>)}</select></label><label className="field-label">Срок сдачи <span className="font-normal text-stone-400">(необязательно)</span><input type="datetime-local" className="field-control mt-2" value={assignment.due_at} onChange={(event) => setAssignment({ ...assignment, due_at: event.target.value })} /></label></div></Dialog>
      <Dialog open={Boolean(selectedAssignment)} onClose={() => setSelectedAssignment(null)} title={selectedAssignment?.title || ''} description={`${selectedAssignment?.completed_students || 0} из ${selectedAssignment?.total_students || 0} учеников выполнили`} footer={<Button variant="ghost" onClick={() => setSelectedAssignment(null)}>Закрыть</Button>}><ProgressBar value={selectedAssignment?.progress || 0} label="Выполнение класса" /></Dialog>
      <Dialog open={Boolean(deleteTarget)} onClose={() => setDeleteTarget(null)} title="Удалить объявление?" description="Ученики больше не увидят его в ленте класса." footer={<><Button variant="ghost" onClick={() => setDeleteTarget(null)}>Отмена</Button><Button variant="danger" loading={actionLoading} onClick={removeAnnouncement}>Удалить</Button></>}><p className="rounded-2xl bg-stone-100 p-4 font-bold">{deleteTarget?.title}</p></Dialog>
