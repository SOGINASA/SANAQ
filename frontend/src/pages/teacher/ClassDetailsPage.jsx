...
import { ClipboardPlus, Copy, Download, FileUp, Megaphone, Pin, Search } from 'lucide-react';
...
export function ClassDetailsPage() {
  ...
  const createAssignment = async () => {
    setActionLoading(true); setError('');
    try {
      const materialId = assignment.file ? await classroomApi.uploadWorkbook(assignment.file) : null;
      const assignmentPayload = {
        title: assignment.title, module_id: assignment.module_id,
        due_at: assignment.due_at, include_workbook: assignment.include_workbook,
      };
      const response = await teacherApi.createAssignment({
        ...assignmentPayload, class_id: classId, material_id: materialId,
  ...
  return (
    <div className="mx-auto max-w-7xl animate-rise">
      ...
      <Dialog open={assignmentOpen} onClose={() => setAssignmentOpen(false)} title={t('classWorkspace.newAssignment')} description={t('classWorkspace.assignmentDescription')} footer={<><Button variant="ghost" onClick={() => setAssignmentOpen(false)}>{t('classWorkspace.cancel')}</Button><Button loading={actionLoading} disabled={!assignment.title.trim() || !assignment.module_id} onClick={createAssignment}>{t('classWorkspace.assign')}</Button></>}><div className="grid gap-4"><label className="field-label">{t('classWorkspace.name')}<input className="field-control mt-2" value={assignment.title} onChange={(event) => setAssignment({ ...assignment, title: event.target.value })} placeholder={t('classWorkspace.namePlaceholder')} /></label><label className="field-label">{t('classWorkspace.module')}<select className="field-control mt-2" value={assignment.module_id} onChange={(event) => setAssignment({ ...assignment, module_id: event.target.value })}>{modules.map((item) => <option key={item.id} value={item.id}>{item.title}</option>)}</select></label><fieldset className="rounded-2xl border border-lavender-200 bg-lavender-50 p-4"><legend className="px-2 text-sm font-extrabold text-lavender-800">{t('classWorkspace.workbook')}</legend><label className="flex min-h-11 cursor-pointer items-start gap-3"><input type="checkbox" className="mt-0.5 h-5 w-5 shrink-0 accent-lavender-600" checked={assignment.include_workbook} onChange={(event) => setAssignment({ ...assignment, include_workbook: event.target.checked, file: event.target.checked ? assignment.file : null })} /><span><strong className="block text-sm">{t('classWorkspace.generatedWorkbook')}</strong><span className="mt-1 block text-xs leading-5 text-stone-500">{t('classWorkspace.generatedWorkbookHint')}</span></span></label>{assignment.include_workbook && <label className="mt-4 block cursor-pointer rounded-2xl border border-dashed border-lavender-300 bg-paper p-4 text-center transition hover:border-lavender-500"><FileUp className="mx-auto h-6 w-6 text-lavender-600" /><span className="mt-2 block text-sm font-extrabold">{assignment.file?.name || t('classWorkspace.uploadWorkbook')}</span><span className="mt-1 block text-xs text-stone-500">{t('classWorkspace.uploadWorkbookHint')}</span><input type="file" accept="application/pdf,.pdf" className="sr-only" onChange={(event) => { const file = event.target.files?.[0] || null; if (file && (file.type !== 'application/pdf' || file.size > 10 * 1024 * 1024)) { setError(t('classWorkspace.workbookInvalid')); event.target.value = ''; return; } setAssignment({ ...assignment, file }); }} /></label>}</fieldset><label className="field-label">{t('classWorkspace.deadline')} <span className="font-normal text-stone-400">{t('classWorkspace.optional')}</span><input type="datetime-local" className="field-control mt-2" value={assignment.due_at} onChange={(event) => setAssignment({ ...assignment, due_at: event.target.value })} /></label></div></Dialog>
