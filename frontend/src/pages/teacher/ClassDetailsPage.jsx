...
export function ClassDetailsPage() {
  ...
  return (
    <div className="mx-auto max-w-7xl animate-rise">
      ...
      <Dialog open={Boolean(selectedAssignment)} onClose={() => setSelectedAssignment(null)} title={selectedAssignment?.title || ''} description={t('classWorkspace.completed', { completed: selectedAssignment?.completed_students || 0, total: selectedAssignment?.total_students || 0 })} footer={<Button variant="ghost" onClick={() => setSelectedAssignment(null)}>{t('classWorkspace.close')}</Button>}><ProgressBar value={selectedAssignment?.progress || 0} label={t('classWorkspace.classCompletion')} /></Dialog>
