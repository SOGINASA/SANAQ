...
import { AssignmentProgressDetails } from '../../features/classroom';
...
export function TeacherAssignmentsPage() {
  ...
  return <div className="mx-auto max-w-6xl animate-rise">
    ...
    <Dialog open={Boolean(selected)} onClose={() => setSelected(null)} title={selected?.title || ''} description={t('assignmentProgress.summary', { completed: selected?.completed_students || 0, started: selected?.started_students || 0, total: selected?.total_students || 0 })} footer={<Button variant="ghost" onClick={() => setSelected(null)}>{t('assignmentsPage.close')}</Button>}><AssignmentProgressDetails assignment={selected} /></Dialog>
