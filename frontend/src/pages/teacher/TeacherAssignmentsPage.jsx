...
export function TeacherAssignmentsPage() {
  ...
  return <div className="mx-auto max-w-6xl animate-rise">
    ...
    <Dialog open={Boolean(selected)} onClose={() => setSelected(null)} title={selected?.title || ''} description={`${selected?.completed_students || 0} завершили · ${selected?.started_students || 0} начали · ${selected?.total_students || 0} всего`} footer={<Button variant="ghost" onClick={() => setSelected(null)}>{t('assignmentsPage.close')}</Button>}><AssignmentProgressDetails assignment={selected} /></Dialog>
