...
export function AdminUsersPage() {
  ...
  const load = useCallback(async (requestedPage = page) => {
    ...
    try {
      ...
      if (!Array.isArray(response.data.items) || !Number.isInteger(response.data.total)) throw new Error(t('adminRuntime.usersInvalid'));
  ...
  const runAction = async (action, message, close, requestedPage = page) => {
    setActionLoading(true); setError('');
    try { await action(); close?.(); setToast(t(message)); await load(requestedPage); }
    catch (requestError) { setError(requestError.message); }
    finally { setActionLoading(false); }
  };
  const createUser = () => runAction(() => adminApi.createUser(form), 'adminUsers.created', () => { setCreateOpen(false); setForm(emptyUser); setPage(1); }, 1);
