...
export function LearningPathPage() {
  ...
  return <div className="mx-auto max-w-6xl animate-rise">
    ...
    {path && <>
      ...
      <div className="mt-7 grid gap-4">{path.steps?.map((step) => { const done = step.status === 'completed'; const available = step.status === 'available'; return <Card key={step.id} className={`p-5 ${available ? 'border-lavender-300 bg-lavender-50' : ''}`}><div className="flex flex-col gap-4 sm:flex-row sm:items-center"><span className={`grid h-12 w-12 shrink-0 place-items-center rounded-2xl ${done ? 'bg-mint-100 text-mint-700' : available ? 'bg-lavender-600 text-white' : 'bg-stone-100 text-stone-400'}`}>{done ? <Check className="h-5 w-5" /> : available ? <Target className="h-5 w-5" /> : <LockKeyhole className="h-5 w-5" />}</span><div className="min-w-0 flex-1"><p className="text-xs font-extrabold uppercase tracking-wider text-stone-400">{t('learningPath.step', { number: step.order })}</p><h2 className="mt-1 break-words text-lg font-extrabold">{step.skill_name}</h2><p className="mt-1 break-words text-sm text-stone-500">{step.reason}</p></div>{available && <Button className="w-full sm:w-auto" variant="outline" onClick={() => navigate(`/student/task/${step.task_id}?path=${path.id}`)}>{t('learningPath.open')}</Button>}</div></Card>; })}</div>
    </>}
