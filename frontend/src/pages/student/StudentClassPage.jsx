...
import { ArrowRight, BookOpenCheck, GraduationCap, School, Users } from 'lucide-react';
...
export function StudentClassPage() {
  ...
  return <div className="mx-auto max-w-5xl animate-rise">
    ...
    <section className="mt-8" aria-labelledby="teacher-materials-title">
      <div className="mb-5 flex items-end justify-between gap-4"><div><p className="eyebrow">{t('studentClass.assignmentsEyebrow')}</p><h2 id="teacher-materials-title" className="mt-2 text-2xl font-extrabold">{t('studentClass.teacherMaterials')}</h2><p className="mt-2 text-sm text-stone-500">{t('studentClass.teacherMaterialsDescription')}</p></div><span className="hidden shrink-0 text-sm text-stone-500 sm:block">{t('studentClass.materialsCount', { count: assignments.length })}</span></div>
      {assignments.length > 0 ? <ClassFeed assignments={assignments} onOpenAssignment={openAssignment} /> : <Card className="border-dashed p-8 text-center"><BookOpenCheck className="mx-auto h-7 w-7 text-stone-400" /><h3 className="mt-4 text-lg font-extrabold">{t('studentClass.noMaterials')}</h3><p className="mt-2 text-sm text-stone-500">{t('studentClass.noMaterialsDescription')}</p></Card>}
    </section>
    {announcements.length > 0 && <section className="mt-9" aria-labelledby="class-announcements-title"><div className="mb-5"><p className="eyebrow">{t('studentClass.latest')}</p><h2 id="class-announcements-title" className="mt-2 text-2xl font-extrabold">{t('studentClass.announcements')}</h2></div><ClassFeed announcements={announcements} /></section>}
  </div>;
}
