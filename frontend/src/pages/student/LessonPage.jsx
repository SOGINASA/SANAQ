    ...
    {module && <div className="grid gap-6 lg:grid-cols-[minmax(0,1fr)_300px]">
      <article className="min-w-0"><div className="mb-6"><p className="eyebrow">{module.title}</p><h1 className="page-title mt-3 break-words">{lesson?.title || t('lesson.fallbackTitle')}</h1><ProgressBar className="mt-5" value={progress} label={t('lesson.progress', { completed: completed.length, total: lessons.length })} /></div>
        ...
        {lesson && <Card className="p-6 sm:p-9"><div className="flex flex-wrap items-center justify-between gap-3"><h2 className="text-2xl font-extrabold">{t('lesson.mainIdea')}</h2><SpeechControls text={[lesson.title, lesson.theory, lesson.example].filter(Boolean).join('. ')} label={t('lesson.read')} /></div><p className="mt-5 whitespace-pre-wrap text-lg leading-8 text-stone-700">{lesson.theory}</p>{lesson.example && <div className="my-7 whitespace-pre-wrap rounded-3xl bg-ink p-7 text-center font-display text-2xl font-semibold text-white">{lesson.example}</div>}
          {lessonTasks.length > 0 && <section className="mt-9 rounded-3xl bg-stone-100 p-4 sm:p-5"><div className="flex flex-wrap items-center justify-between gap-3"><div><h3 className="text-lg font-extrabold">Практика урока</h3><p className="mt-1 text-sm text-stone-500">Выполнено {completedLessonTasks} из {lessonTasks.length}</p></div><span className={`rounded-full px-3 py-1.5 text-xs font-bold ${allLessonTasksComplete ? 'bg-mint-100 text-mint-700' : 'bg-paper text-stone-600'}`}>{allLessonTasksComplete ? 'Всё готово' : 'Нужно выполнить все'}</span></div><div className="mt-4 grid gap-2">{lessonTasks.map((taskItem, index) => { const done = completedTaskIds.includes(taskItem.id); return <button key={taskItem.id} type="button" onClick={() => navigate(`/student/task/${taskItem.id}?module=${moduleId}`)} className={`flex min-h-14 w-full cursor-pointer items-center gap-3 rounded-2xl border p-3 text-left transition ${done ? 'border-mint-200 bg-mint-100' : 'border-stone-200 bg-paper hover:border-lavender-300'}`}><span className={`grid h-9 w-9 shrink-0 place-items-center rounded-xl ${done ? 'bg-mint-700 text-white' : 'bg-lavender-100 text-lavender-700'}`}>{done ? <CheckCircle2 className="h-5 w-5" /> : <Play className="h-4 w-4" />}</span><span className="min-w-0 flex-1"><strong className="block">Задание {index + 1}</strong><span className="mt-0.5 block break-words text-sm text-stone-500">{taskItem.prompt}</span></span><span className="shrink-0 text-sm font-bold">{done ? 'Готово' : 'Начать'}</span></button>; })}</div></section>}
          {!allLessonTasksComplete && <p className="mt-4 flex items-start gap-2 rounded-2xl bg-amber-50 p-4 text-sm font-semibold text-amber-900"><Circle className="mt-0.5 h-4 w-4 shrink-0" /> Сначала завершите все задания практики — тогда урок и назначение обновятся автоматически.</p>}
