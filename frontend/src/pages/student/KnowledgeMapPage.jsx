...
import { Button, Card, Dialog, PageSkeleton, ProgressBar } from '../../shared/ui';
...
export function KnowledgeMapPage() {
  ...
  if (loading) return <PageSkeleton cards={3} label={t('knowledgeMap.loading')} />;
  ...
  return <div className="mx-auto max-w-6xl animate-rise"><div><p className="eyebrow">{t('knowledgeMap.eyebrow')}</p><h1 className="page-title mt-3">{t('knowledgeMap.title')}</h1><p className="mt-3 text-stone-600">{t('knowledgeMap.description')}</p></div>
    ...
    {nextStep && <Card className="mt-7 border-lavender-300 bg-lavender-50 p-6 sm:p-8"><p className="eyebrow">{t('knowledgeMap.recommended')}</p><h2 className="mt-2 text-2xl font-extrabold">{nextStep.skill_name}</h2><p className="mt-3 text-stone-600">{nextStep.reason}</p><Button className="knowledge-next-signal mt-6" onClick={() => navigate(`/student/task/${nextStep.task_id}?path=${pathId}`)}>{t('knowledgeMap.continue')} <ArrowRight className="h-5 w-5" /></Button></Card>}
    <div className="mt-7 grid gap-6 lg:grid-cols-3">{groups.map(([status, title, Icon, tone]) => { const items = nodes.filter((node) => normalizedStatus(node) === status); return <section key={status} className="constellation-lane"><div className="mb-3 flex items-center gap-2"><Icon className="h-5 w-5 text-lavender-600" /><h2 className="text-lg font-extrabold">{title}</h2><span className="rounded-full bg-stone-100 px-2 py-1 text-xs font-bold">{items.length}</span></div><div className="constellation-track grid gap-3">{items.map((node, index) => <button key={node.id} onClick={() => setSelected(node)} style={{ '--node-delay': `${index * 85}ms` }} className={`constellation-node constellation-node--${status} rounded-3xl border border-stone-200 bg-paper p-5 text-left transition hover:border-lavender-300`}><div className="flex items-start justify-between gap-3"><strong>{node.name}</strong><span className="text-sm font-bold text-stone-500">{Math.round(node.mastery * 100)}%</span></div><p className="mt-2 text-xs text-stone-500">{node.topic_name}</p><ProgressBar className="mt-4" value={Math.round(node.mastery * 100)} tone={tone} /></button>)}</div></section>; })}</div>
