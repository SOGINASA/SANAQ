...
const groups = [
  ['mastered', 'Освоено', CheckCircle2, 'mint'],
  ['learning', 'В процессе', CircleDot, 'violet'],
  ['locked', 'Пока закрыто', LockKeyhole, 'violet'],
];
...
export function KnowledgeMapPage() {
  ...
  if (loading) return <div className="mx-auto max-w-6xl py-16 text-center font-bold">Загружаем карту знаний…</div>;

  return <div className="mx-auto max-w-6xl animate-rise"><div><p className="eyebrow">Карта знаний</p><h1 className="page-title mt-3">Что уже крепко, а что открыть дальше</h1><p className="mt-3 text-stone-600">Mastery и блокировки рассчитаны backend по сохранённым ответам.</p></div>
    {error && <div className="mt-6 rounded-2xl border border-red-200 bg-red-50 p-4 text-red-900">{error}</div>}
    {nextStep && <Card className="mt-7 border-lavender-300 bg-lavender-50 p-6 sm:p-8"><p className="eyebrow">Рекомендуемый узел</p><h2 className="mt-2 text-2xl font-extrabold">{nextStep.skill_name}</h2><p className="mt-3 text-stone-600">{nextStep.reason}</p><Button className="mt-6" onClick={() => navigate(`/student/task/${nextStep.task_id}?path=${pathId}`)}>Продолжить <ArrowRight className="h-5 w-5" /></Button></Card>}
    ...
    {!nodes.length && !error && <Card className="mt-7 p-8 text-center">Карта пока пуста. Завершите диагностику.</Card>}
    <Dialog open={Boolean(selected)} onClose={() => setSelected(null)} title={selected?.name || ''} description={selected?.topic_name || ''} footer={<><Button variant="ghost" onClick={() => setSelected(null)}>Закрыть</Button>{selected && normalizedStatus(selected) !== 'locked' && nextStep?.skill_id === selected.id && <Button onClick={() => navigate(`/student/task/${nextStep.task_id}?path=${pathId}`)}>{normalizedStatus(selected) === 'mastered' ? <><RotateCcw className="h-5 w-5" /> Повторить</> : <>Продолжить <ArrowRight className="h-5 w-5" /></>}</Button>}</>}><p className="text-stone-600">{selected?.mastery_label}. Уверенность данных: {Math.round((selected?.confidence || 0) * 100)}%.</p>{selected?.blocked_by?.length > 0 && <p className="mt-3 text-sm text-stone-500">Сначала нужно освоить: {selected.blocked_by.join(', ')}.</p>}</Dialog>
  </div>;
}
