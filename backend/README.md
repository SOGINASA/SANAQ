...
и проверяются автоматическим контрактным тестом. Реализованные функции возвращают рабочие
данные; маршруты будущих доменов явно отвечают `501 FEATURE_NOT_IMPLEMENTED`, пока их
бизнес-логика не готова.
...
| POST | `/api/v1/ai/explanations`, `/api/v1/ai/hints` | Безопасный AI-fallback |
...
Оставшиеся 55 URL из `API_ROUTES.md` находятся в `routes/contract_placeholders.py`. Это
временный реестр маршрутов, а не реализация функций. При добавлении очередного домена его
маршрут нужно перенести из реестра в отдельный blueprint; контрактный тест подтвердит, что
URL и HTTP-метод не потерялись.
...
задание → AI-fallback → завершение попытки → карта и прогресс
```
